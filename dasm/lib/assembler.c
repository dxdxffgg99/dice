...
static libdice_word_t dasm_assemble_line(libdice_word_t rdwr_dst[], const libdice_word_t c_dst_len, const struct dasm_parsed_line *rd_parsed_line)
{
	...
	for (tmp_operand_cnt=0; tmp_operand_cnt<rd_parsed_line->m_operand_cnt; ++tmp_operand_cnt) {
		...
		if (end == rd_parsed_line->m_operands[tmp_operand_cnt].m_text) {
			return DASM_ERR_RET;	/* not number */
		} else if (*end != '\0') {
			return DASM_ERR_RET;	/* trash after number */
		}
...
DICEIMPL libdice_word_t dasm_assemble_programme(libdice_word_t rdwr_programme[], const libdice_word_t c_programme_len, 
					const struct dasm_parsed_line rd_parsed_lines[], const libdice_word_t c_parsed_lines_len)
{
	...
	for (parsed_line_cnt=0; parsed_line_cnt<c_parsed_lines_len; ++parsed_line_cnt) {
		tmp = dasm_assemble_line(&rdwr_programme[programme_word_cnt], c_programme_len-programme_word_cnt, &rd_parsed_lines[parsed_line_cnt]);
