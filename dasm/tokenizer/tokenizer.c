...
static void libdasm_init_token_line(struct libdasm_token_line *rdwr_token_line)
{
	...
	memset(rdwr_token_line->m_token_line, 0, sizeof(struct libdasm_token) * LIBDASM_TOKEN_MAX_CNT_PER_LINE);	/* not essential */
}
...
static bool libdasm_create_new_token(struct libdasm_token_line *rdwr_token_line)
{
	...
	memset(rdwr_token_line->m_token_line[rdwr_token_line->m_token_cnt].m_token, 0, LIBDASM_TOKEN_MAX_LEN);
...
static inline void libdasm_insert_token_char(struct libdasm_token_line *rdwr_token_line, const char c_c)
{
	...
	for (i=0; i<LIBDASM_TOKEN_MAX_LEN; i++) {
		if (rdwr_token_line->m_token_line[rdwr_token_line->m_token_cnt-1].m_token[i] == '\0') {
			assert(i+1 < LIBDASM_TOKEN_MAX_LEN);
			rdwr_token_line->m_token_line[rdwr_token_line->m_token_cnt-1].m_token[i] = c_c;
			rdwr_token_line->m_token_line[rdwr_token_line->m_token_cnt-1].m_token[i+1] = '\0';
			break;
		}
	}
}
...
static inline void libdasm_set_token_type(struct libdasm_token_line *rdwr_token_line, const enum e_libdasm_token_type c_token_type)
{
	...
	rdwr_token_line->m_token_line[rdwr_token_line->m_token_cnt-1].m_token_type = c_token_type;
}
...
static libdice_word_t libdasm_tokenize_programme(struct libdasm_token_line rdwr_token_table[], const libdice_word_t c_token_table_len, const char *rd_src)
{
	...
	while (pc < src_len) {
		...
		pc += libdasm_tokenize_line(&rdwr_token_table[token_table_cnt], rd_src+pc);
		token_table_cnt++;
	} 
